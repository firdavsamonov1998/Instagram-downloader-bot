...
import com.example.moduls.ApiModules;
import com.example.servise.CookieService;
...
public class MainController {
    ...
    public MainController(MyTelegramBot myTelegramBot, CookieService cookieService) {
...
//            String urlImage = imageObject.getString("src");
    ...
    public void getUrlAndSendVideoReel(Message message, String url) {
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("https://api.bhawanigarg.com/social/instagram/?link="+url + "&cookie=" + cookieService.cookie())).build();
        HttpResponse<String> response = null;
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                if (object.getInt("media_type") == 1){
                    String urlPhoto = object.getString("media_image");
                   myTelegramBot.send(SendMsg.sendPhoto(message.getChatId(),
                           url + "\n\n" +
                                   "\uD83D\uDCE5 @isnta_video_bot",
                           urlPhoto));
                } else if (object.getInt("media_type") == 2) {
                    String video = object.getString("media_video");
                    myTelegramBot.send(SendMsg.sendVideo(message.getChatId(),
                            video,url));
                }
            }
